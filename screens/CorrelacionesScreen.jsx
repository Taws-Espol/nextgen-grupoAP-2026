...
function CorrelacionesScreen({ team, initialProgress, onProgress, onComplete }) {
  ...
  if (step === 1) {
    ...
    const allPicked = Object.keys(picks).length >= mCount;
    ...
    if (allPicked) return (
      <div style={{ padding:28, flex:1, overflowY:"auto", display:"flex", flexDirection:"column", gap:20 }}>
        ...
        <div>
          ...
          <p style={{ color:C.muted, fontSize:14, margin:0 }}>Cada integrante eligió una variable. Ahora el equipo acuerda cuál va en X y cuál en Y.</p>
        </div>
        ...
        <Card style={{ padding:24 }} glow={C.cyan}>
          <div style={{ display:"flex", gap:16, flexDirection:"column" }}>
            ...
            <div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(140px, 1fr))", gap:8 }}>
                {pickedKeys.filter(k => k !== xVar).map(k => {
                  const col = COLUMNS.find(c => c.key === k);
                  return (
    ...
    return (
      <div style={{ padding:28, flex:1, overflowY:"auto", display:"flex", flexDirection:"column", gap:20 }}>
        ...
        <TurnBadge idx={pickerIdx} />

        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(120px, 1fr))", gap:8 }}>
          {members.slice(0, mCount).map((m, i) => {
            const picked = picks[i];
            const isCurrent = i === pickerIdx;
            const col = COLUMNS.find(c => c.key === picked);
            return (
              <div key={i} style={{ padding:"10px 12px", borderRadius:10,
                background:picked?`${C.green}12`:isCurrent?`${C.yellow}08`:"rgba(255,255,255,0.02)",
                border:`1px solid ${picked?C.green:isCurrent?C.yellow:C.border}`, transition:"all 0.3s" }}>
                <div style={{ fontSize:11, fontWeight:700, color:picked?C.green:isCurrent?C.yellow:C.muted, marginBottom:4 }}>{memberName(i)}</div>
                {picked
                  ? <div style={{ fontSize:13, fontWeight:700, color:C.text }}>{col?.emoji} {col?.label}</div>
                  : <div style={{ fontSize:11, color:C.dim }}>{isCurrent?"Eligiendo...":"Esperando turno"}</div>
                }
              </div>
            );
          })}
        ...
        <Card style={{ padding:22 }}>
          <div style={{ fontSize:13, fontWeight:700, color:C.yellow, marginBottom:16 }}>
            {memberName(pickerIdx)}, ¿qué variable te parece más interesante?
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(150px, 1fr))", gap:12 }}>
            {NUMERIC_COLS.map(col => {
              const taken = pickedKeys.includes(col.key);
              return (
                <button key={col.key}
                  onClick={() => {
                    if (taken) return;
                    setPicks(p => ({ ...p, [pickerIdx]: col.key }));
                    setPickerIdx(i => Math.min(i + 1, mCount - 1));
                    giveXP(15);
                  }}
                  disabled={taken}
                  style={{ padding:"18px 14px", border:`2px solid ${taken?C.border:C.yellow}50`,
                    borderRadius:14, background:taken?"rgba(255,255,255,0.02)":`${C.yellow}08`,
                    cursor:taken?"not-allowed":"pointer", textAlign:"center", opacity:taken?0.3:1, transition:"all 0.2s" }}>
                  <div style={{ fontSize:30, marginBottom:8 }}>{col.emoji}</div>
                  <div style={{ fontSize:13, fontWeight:700, color:taken?C.dim:C.text, lineHeight:1.3 }}>{col.label}</div>
                </button>
              );
            })}
          </div>
        ...
        <p style={{ color:C.dim, fontSize:12, textAlign:"center" }}>{Object.keys(picks).length}/{mCount} integrantes han elegido</p>
      </div>
    );
  }
  ...
  if (step === 3) {
    const valid = YT_DATA.filter(d => !isNaN(d[xVar]) && !isNaN(d[yVar]));
  ...
  if (step === 4) {
    const q = ANALYSIS_QS[analysisQIdx];
          ...
          {!answered ? (
            ...
            <div>
              ...
              <div style={{ display:"flex", justifyContent:"flex-end" }}>
                {analysisQIdx < ANALYSIS_QS.length - 1
      ...
      <div style={{ padding:28, flex:1, overflowY:"auto", display:"flex", flexDirection:"column", gap:20 }}>
        ...
        <div style={{ padding:"12px 16px", background:`${C.yellow}10`, border:`1px solid ${C.yellow}25`, borderRadius:12, display:"flex", gap:10 }}>
          ...
          <p style={{ fontSize:13, color:C.muted, lineHeight:1.6, margin:0 }}>
            <strong style={{ color:C.yellow }}>Tawsito:</strong> "Una sola oración clara sobre <strong style={{ color:C.text }}>{xCol?.label}</strong> y <strong style={{ color:C.text }}>{yCol?.label}</strong>. ¡Todos deben estar de acuerdo antes de enviarla! 📝"
          </p>
        </div>
        ...
        {conclusionText.trim().length >= 10 && (
          <Card style={{ padding:22 }} glow={C.green}>
            <div style={{ fontSize:14, fontWeight:800, marginBottom:4 }}>✅ Aprobación del equipo ({approvedCount}/{mCount})</div>
            <p style={{ color:C.muted, fontSize:13, marginBottom:16 }}>Cada integrante aprueba antes de enviar a Tawsito.</p>
              ...
              {members.slice(0, mCount).map((m, i) => {
                ...
                return (
                  <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
                    ...
                    background:approved?`${C.green}12`:"rgba(255,255,255,0.03)",
                    border:`1px solid ${approved?C.green:C.border}`, transition:"all 0.3s" }}>
                    <span style={{ fontSize:13, fontWeight:700, color:approved?C.green:C.text }}>{memberName(i)}</span>
                    <button
                      onClick={() => { if (!approved) { setApprovals(p => ({ ...p, [i]: true })); giveXP(25); } }}
                      disabled={approved}
                      style={{ border:"none", borderRadius:8, cursor:approved?"default":"pointer",
                        padding:"7px 14px", background:approved?`${C.green}30`:C.purple,
                        color:approved?C.green:"#fff", fontWeight:700, fontSize:12, transition:"all 0.2s" }}>
            ...
            {allApproved && (
              <div>
                <div style={{ padding:14, background:`${C.green}15`, border:`1px solid ${C.green}30`, borderRadius:10, marginBottom:16, textAlign:"center" }}>
